...
//==========================================================================================
{
    ...
    //----------------------------------------------------------------------------------------  
    delay(10);
    touch_result = digitalRead(touch_sensor);     // 터치 센서 읽기
}
...
{  
    ...
    delay(10);
    if(touch_result == HIGH)                      // 터치센서가 눌러졌으면
    {
        servo.attach(servo_motor);                // 서보모터 제어 시작
        servo.write(50);                          // 서보모터를 기본(닫힘) 상태로 초기화
    
        servo.write(0);                           // 서보모터를 열림 상태로 만듦
        delay(500);                               // 서보모터 열리는 시간과 모이가 내려가는 시간
        servo.write(50);                          // 서보모터를 닫기
        delay(100);                               // 서보모터 닫는 시간
        servo.detach();                           // 서보모터 제어 시작
        count ++;                                 // 모이 준 횟수
    }
}
